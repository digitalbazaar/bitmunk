...
(function($)
{
    ...
    */
   bitmunk.events.init = function(task)
   {
      var timer = +new Date();
      task.block();
    ...
    */
   bitmunk.events.cleanup = function(task)
   {
      // stop polling
      state.poll = false;
    ...
    */
   bitmunk.events.createObservers = function(num, options)
   {
    ...
    */
   bitmunk.events.deleteObservers = function(observerIds, options)
   {
      // set parameter defaults
      options = $.extend(
      {
    ...
    */
   bitmunk.events.registerObservers = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
    ...
    */
   bitmunk.events.unregisterObservers = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
    ...
    */
   bitmunk.events.subscribe = function(options)
   {
    ...
    *        complete: a callback for when the subscription completes.
    */
   bitmunk.events.multiSubscribe = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
      ...
      // build observers request in reverse because the event groups are
      // popped off the end of the event group array (this way the order
      // of the observer IDs matches the order of the event groups)
      var obs = [];
      var eg = options.eventGroups;
      while(eg.length > 0)
      {
         // use id '0' to create observers on-the-fly
         var group = eg.pop();
         obs.push({
            id: '0',
            events: group.events,
    ...
    */
   bitmunk.events.sendEvents = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
    ...
    *        complete: a callback for when the sending completes.
    */
   bitmunk.events.receiveEvents = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
    ...
    */
   bitmunk.events.addDaemonEvents = function(options)
   {
      // set parameter defaults
      options = $.extend(
      {
